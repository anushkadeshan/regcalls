    ...
    watch : {
        params : {
            handler: throttle(function () {
                ...
                this.$inertia.get(this.route('users.index'), this.params, {
                    replace :true,
                    preserveState : true
                })
            },150),
...
<template lang="">
    <div class="my-4" v-if="$page.props.currentPermissions.includes('View User')">
        <div class="md:flex my-4 justify-between">
            ...
            <div class="md:text-right md:mt-0 mt-2">
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="py-3 px-6">
                            <span @click="sort('email')" class="flex space-x-4">
                                {{$t('Email')}}
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 7.5L7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5" />
                                </svg>
                        ...
                        <th scope="col" class="py-3 px-6">
                            <span @click="sort('created_at')" class="flex space-x-4">
                                {{$t('Joined At')}}
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 7.5L7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5" />
                                </svg>
                ...
                <tbody>
                    <tr v-if="users.data.length >0"  v-for="(user, key) in users.data" :key="user.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="py-2 px-6">
                            <div class="inline-flex rounded-md shadow-sm">
                                ...
                                <Link :href="route('users.edit',[user.id])" class="px-2 py-1 bg-gradient-to-r from-green-500 to-green-600 hover:opacity-80 text-white text-sm font-medium">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>

                                </Link>
                                <a @click="showModal(user.id)" class="px-2 py-1 bg-gradient-to-r from-red-600 to-red-700 hover:opacity-80 text-white text-sm font-medium rounded-r-md ">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <Pagination :links="users.links" :to_items="users.to" :from_items="users.from" :total_items="users.total"></Pagination>
    </div>
    <div v-else>

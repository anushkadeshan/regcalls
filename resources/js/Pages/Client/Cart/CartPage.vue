        ...
        <div class="grid grid-cols-12 items-start gap-6 mx-8 my-4 pt-4 pb-16">
            <div class="col-span-9 space-y-4">
                <div v-for="(product, index) in products" :key="index" class="flex items-center justify-between border gap-6 p-4 border-gray-200 rounded">
                    ...
                    <input type="number" @change="changeQuantity($event,product.product_id)" v-model="product.quantity" class="w-20 border border-rose-500 focus:ring-rose-500 focus:border-rose-500 text-gray-500" min="1" :max="product.max_quantity">
                    <div class="text-rose-500 text-md font-semibold">{{product.price}}</div>
                    <div class="text-rose-500 text-md font-semibold">{{product.price*product.quantity}}</div>
            ...
            <div class="col-span-3 space-y-4">
                <div class="border border-gray-200 rounded">
                    ...
                    <button class="px-4 py-2 rounded-b text-white w-full bg-rose-500 hover:bg-rose-400">
                        {{$t('Checkout')}}
                    </button>
                </div>
            </div>
